...
func (t *InvoTracker) AfterAgent(ctx agent.Context) bool {
	...
	slog.InfoContext(
		...
		slog.GroupAttrs(
			"tokens",
			slog.Int("input", tokens.Input),
			slog.Int("output", tokens.Output),
			slog.Int("total", tokens.Total),
		),

...
> **并发契约**：自动会话族 API 必须按 userId 串行化（多实例部署要跨实例），`Chat` 自身不加锁。互斥可直接复用 `userlock` 包——按 key 互斥的 Redis 分布式锁，带续期、失联取消和有界等待（`userlock.ErrBusy`），钉钉渠道即用它实现；否则并发消息会让事件交错、与 `ResetAutomatic` 并发的运行可能把回答写进已被轮换掉的旧会话。详见 `llmchat.Chat` 的文档。
